...
module.exports = class FileProcessor {
  ...
  extractLaps() {
    ...
    for (let i = 0; i < t.length - 1; i++) {
      ...
      const k = this.crossesStartFinishLine(t[i], t[i + 1]);
      if (k != false) {
  ...
  processLapFile(path) {
    ...
    for (let i = 0; i < samples.length - 1 - 1; i++) {
      const k = this.crossesStartFinishLine(samples[i], samples[i + 1]);
      if (k != false) {
  ...
  crossesStartFinishLine(from, to) {
    ...
    return this.intersect(
      ...
      to.lon,
      to.lat,
      this.finishline.start.lon,
      this.finishline.start.lat,
      this.finishline.end.lon,
      this.finishline.end.lat,
    );
  }
